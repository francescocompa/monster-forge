...
Rejected: DM-edited entries and the both-option — free-text boon entries would have to ride the share cfg and be sanitized phone-side like /refs (a new hostile text surface), and Francesco explicitly scoped this round to the toggle. Not re-proposed without him. **Reopened by him and RESOLVED 2026-08-14 → D-052: prose-only custom entries are approved** (a trait sentence, never structured fx).
...
~~Open: making the boon TEXT editable stays tabled (D-035), now explicitly deferred rather than undesigned.~~ **CLOSED 2026-08-14 → D-052** (prose-only).
