...
function renderAdvDetail(){
  ...
  d.innerHTML=`<div class="adv-topbar" data-advcolor="${a.id}" title="Adventure colour"${a.color?` style="background:linear-gradient(90deg,${a.color},color-mix(in srgb,${a.color} 55%,#000))"`:""}></div>
    ...
    <div class="section-label" id="partyHead"><span>Party roster${a.party.length?` <span class="pc-count2">${a.party.length}</span>`:""}</span><span class="sl-acts"><button class="lvlup-btn" id="partyLvlUp" title="Level up the party">${ARROW_TREND_UP}<span class="lvlup-txt">Level Up</span></button>${a.crew?`<button class="lvlup-btn" id="crewShareBtn" title="Share the crew${a.crew.shareId?" (live)":""}">${SHARE_ICON}${a.crew.shareId?'<span class="lvlup-txt">Live</span>':""}</button><button class="lvlup-btn" id="crewSettings" title="Crew generator settings" aria-label="Crew generator settings">${GEN_GEAR_ICON}</button>`:""}</span></div>
  ...
  {const cs=$("#crewSettings");if(cs)cs.addEventListener("click",()=>openCrewSettings(a));
   const sh=$("#crewShareBtn");if(sh)sh.addEventListener("click",()=>openCrewShareDialog(a));}
