...
function renderSettings(){
  ...
  body.innerHTML=`<h2 class="set-title">Settings</h2>
    ...
    <div class="set-card">
      ...
      <div class="set-note">When you run a combat, each monster's HP is rolled from its Hit Dice formula or set to its average, and initiative is rolled (1d20 + mod) or taken as a static average (10 + mod). You can re-roll everyone from the combat toolbar. With grouped initiative on, all copies of one enemy entry share a single roll; turn it off to roll each separately. Initiative ties fall to the higher Dexterity. Turn off party HP to hide the HP tracker for player characters. At 0 HP, "down" combatants roll death saves (tracked on the initiative entry) instead of being marked dead outright. You choose whether that applies to players only, anyone, or nobody (monsters are otherwise marked dead). Player editing on the shared initiative (the Share button in combat) is just an Off/On toggle there now — no key to set up.</div>
    </div>
    <div class="set-card">
...
function ingestLibraries(loaded){
  ...
  loaded.forEach(L=>{
    ...
    try{
      ...
      else if(kind==="feat"){const p=parseFeatsJSON(L.json,L.name,state.books);state.feats=state.feats.filter(x=>x._source!==L.name).concat(p);saveFeats();summary.push(`${L.name}: ${p.length.toLocaleString()} feats`);}
